...
public class WompiClient {
    ...
    private String acceptanceTokenFallback(Throwable t) {
        log.error("Fallo al obtener acceptance token: {}", t.getMessage());
    ...
    private WompiTransactionResponse createTransactionFallback(WompiTransactionRequest request, Throwable t) {
        if (t instanceof WebClientResponseException wex && wex.getStatusCode().is4xxClientError()) {
            log.error("Wompi rechazó la transacción con {}: reference={}", wex.getStatusCode(), request.reference());
            throw wex;
        }
        log.error("Circuit breaker / retry agotado al crear transacción: reference={} error={}", request.reference(), t.getMessage());
        throw new RuntimeException("Pasarela de pagos no disponible. Intente más tarde.", t);
    }
    ...
    public WompiTransactionResponse getTransactionByReference(String reference) {
        ...
        WompiTransactionListResponse response = buildClient().get()
                ...
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + wompiConfig.getPrivateKey())
                .retrieve()
                .bodyToMono(WompiTransactionListResponse.class)
                .timeout(Duration.ofSeconds(wompiConfig.getTimeoutSeconds()))
                .block();
        ...
        return new WompiTransactionResponse(response.data().get(0));
    }
}
